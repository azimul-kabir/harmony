...
def upgrade() -> None:
    ...
    if "provider_cache_entries" not in inspector.get_table_names():
        op.create_table(
    ...
    existing_indexes = {
        index["name"]
        for index in sa.inspect(bind).get_indexes("provider_cache_entries")
    }
    if "uq_provider_cache_key" not in existing_indexes:
        op.create_index(
    ...
    if "ix_provider_cache_expiry" not in existing_indexes:
        op.create_index(
            "ix_provider_cache_expiry",
            "provider_cache_entries",
            ["provider", "expires_at"],
        )
